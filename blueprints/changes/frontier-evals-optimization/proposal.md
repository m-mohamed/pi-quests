...
We're Pi-native. The Pi community shares raw session traces, and Quest already has an eval optimizer layer. We can learn from both without introducing a separate runtime.

**The gap:** Quest trace bundles are derived summaries. Community traces are raw Pi sessions. We need Pi-native trace processing, canonical eval storage, and a proposer-driven frontier loop on real long-horizon evals.

## What changes

1. **Pi-native trace format** — Use raw Pi session JSONL directly, don't convert
2. **Canonical eval filesystem** — Store candidates, scores, frontier state, splits, and community stats under `.pi/quests/evals/`
...
- No compatibility shell: operate only from `.pi/quests/evals/`
